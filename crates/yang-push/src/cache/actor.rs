...
//!     handle1.request_tx()
//!         .send(CacheLookupCommand::LookupBySubscriptionInfo { subscription_info, session, tx })
//!         .await?;
//!
...
pub enum CacheLookupCommand {
    ...
    LookupByContentId {
        ...
        tx: async_channel::Sender<(ContentId, Option<Arc<YangLibraryReference>>)>,
    },

    LookupByContentIdOneShot {
        content_id: ContentId,
        tx: oneshot::Sender<(ContentId, Option<Arc<YangLibraryReference>>)>,
    },
}
...
impl std::fmt::Display for CacheLookupCommand {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::LookupBySubscriptionInfoOneShot {
                subscription_info,
                session,
                tx: _,
            } => {
            ...
            Self::LookupByContentId { content_id, tx: _ } => {
                write!(f, "lookup by content id {content_id}")
            }
            Self::LookupByContentIdOneShot { content_id, tx: _ } => {
...
impl<F: YangLibraryFetcher> CacheActor<F> {
    ...
    async fn process_request(&mut self, request: CacheLookupCommand) {
        match request {
            CacheLookupCommand::LookupBySubscriptionInfo {
                subscription_info,
                session,
                tx: sender,
            } => {
            ...
            CacheLookupCommand::LookupByContentId {
                content_id,
                tx: sender,
            } => {
            ...
            CacheLookupCommand::LookupByContentIdOneShot {
                content_id,
                tx: sender,
            } => {
                let otl_tags = [opentelemetry::KeyValue::new(
