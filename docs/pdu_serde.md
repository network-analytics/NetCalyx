...
NetCalyx relies heavily on the rust's rich type system to represent packets in the away that allows the rust compiler
...
2. For constants, use `enum` to list the possible values. For example, if the PDU has a type field with limited set
   of options, then NetCalyx uses an enum to represent them.
...
* NetCalyx uses [nom](https://github.com/geal/nom) as the primary parser.
* In order to deserialize PDUs efficiently, NetCalyx provides a fork
  of [nom_locate](https://github.com/fflorent/nom_locate). See [netcalyx-locate](../crates/locate/README.md).
* [netcalyx-parse-utils](../crates/parse-utils) provides the essential traits to handle PDUs parsing. And
